...
<body>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        ...
        <input type="date" name="fechaIni" id="fechaIni" max=<?php echo date('Y-m-d');
        if (isset($_POST['fechaIni'])) {
            echo " value=" . $_POST['fechaIni'];
        } else {
            echo " value=2023-01-01";
        } ?>>
        <br>
        <label for="fechaFin">Fecha final</label>
        <input type="date" name="fechaFin" id="fechaFin" max=<?php echo date('Y-m-d');
        if (isset($_POST['fechaFin'])) {
            echo " value=" . $_POST['fechaFin'];
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (!empty($_POST['fechaIni']) && !empty($_POST['fechaFin'])) {
            if (strtotime($_POST['fechaFin']) - strtotime($_POST['fechaIni']) < 0) {
                echo "<p>&#9888; La fecha inicial debe ser anterior a la fecha final</p>";
            } else {
            ...
            echo "<p>&#9888; Se deben especificar ambas fechas</p>";
        }
    }
    ?>
</body>
